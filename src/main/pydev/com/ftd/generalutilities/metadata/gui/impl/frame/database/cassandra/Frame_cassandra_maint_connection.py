    ...
    def before_next(self):
        ...
        if not File_processor.verify_file(cassandra_conection_file):
            ...
            if self.__maintain_mode == MATAIN_MODE_NEW and \
                Cassandra_connection_file_processor.verify_connection_name_exist(cassandra_conection_file, self.__input01.get()):
                
                if not askyesno('Warning', 'The connection name is existing, do you confirm to overwrite?'):
                    return False
            Cassandra_connection_file_processor.update_connection_file(cassandra_conection_file, connection_name, connection_param)
